/*
  ...
  - Added the required column `paymentRef` to the `orders` table without a default value. This is not possible if the table is not empty.
...
-- AddForeignKey
ALTER TABLE `order-detail` ADD CONSTRAINT `order-detail_orderId_fkey` FOREIGN KEY (`orderId`) REFERENCES `orders`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `order-detail` ADD CONSTRAINT `order-detail_productId_fkey` FOREIGN KEY (`productId`) REFERENCES `products`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
